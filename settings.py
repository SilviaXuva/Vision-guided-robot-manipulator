...
class Controller:
    def __init__(self, type, loop, Kp = []) -> None:
        self.type = type
        self.loop = loop
        if type == 'cart':
            self.Kp_trans = Kp[0]
...
class Settings:
    ...
    # Controller = Controller('cart', 'open', [8,6])
    Controller = Controller(None, 'open')
    ...
    output_path = fr'.\Output\{datetime.now().strftime("%Y-%m-%d_%Hh%Mm%Ss")}\Trajectory type = {Trajectory.type} - source = {Trajectory.source}\Controller type = {Controller.type} - loop = {Controller.loop}'
